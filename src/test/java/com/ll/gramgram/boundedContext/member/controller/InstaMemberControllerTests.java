...
public class InstaMemberControllerTests {
    ...
    void t001() throws Exception {
        ...
        ResultActions resultActions = mvc // mvc를 통해서
                .perform(get("/instaMember/connect")) //  get 요청으로 수행을 하겠다.  스프링 한테 여기로 접속해라! 라고 시키는것
                .andDo(print()); // 크게 의미 없고, 그냥 확인용

        // THEN // 접속한 결과에 대해서는 이래야 한다고 정의
        resultActions
                .andExpect(handler().handlerType(InstaMemberController.class)) // 핸들러 타입 /member/join으로 맵핑되는 메서드가 있는 컨트롤러가 작동되어야 한다고 정의
                .andExpect(handler().methodName("showConnect")) // /member/join으로 맵핑되는 메서드가 작동되어야 한다고 정의
